...
def get_matches_df(sparse_matrix, name_vector, top=7):
...
def KeepMostSimilarDes(names):
    ...
    if len( names['itemDescription'])<=1:
        ...
        if (len(names)==2): 
            ...
            names0=pd.concat([names1,dfnullindex],axis=0).reset_index(drop=True)
        else:    
            ...
            t1 = time.time()
            matches = awesome_cossim_top(tf_idf_matrix, tf_idf_matrix.transpose(), 20, 0)
            t = time.time()-t1
            print("SELFTIMED:", t)
            matches_df = get_matches_df(matches, company_names, top=7)
...
def addNameOnTop(result,df1):
    
    df1['company name']=df1['name']
    df1=df1.rename(columns={"id":"item_id","description":"itemDescription", "website":"label"})
    ...
    for i in range(len(df1)):    
        for j in range(len(result)):
            if ((df1['company name'][i].strip()==result['company name'][j].strip()) and (len(result)>1) and (df1['company name'][i] not in listnames)):
                ...
                initialname=df1[df1['company name']==df1['company name'][i]].reset_index(drop=True)
                listnames.append(df1['company name'][i])    
                resultname=result[result['company name']==result['company name'][j]].reset_index(drop=True)
               
                df2=pd.concat([initialname, resultname],axis=0).reset_index(drop=True)
                
                df2=df2[['item_id','company name','itemDescription','name','label']]
                df2.fillna('')
...
def searchInKg(listkeyword_of_interest):
    ...
    for keyword_of_interest in tqdm(listkeyword_of_interest):   
        ...
        if dfkg.empty:
            ...
            wordsexclude=["film","music","fans","band","book","unincorporated","beatles","pie","juice","Beatles","cultivar","sea","eat","eating","table","plants","album","actor","play","trial","family","football","rapper","DJ","single","song","girl","boy","girls","boys"]
...
def searchListCompaniesInKg(df):   
    ...
    try:
        ...
        final.columns=({'company name','label','item_id','itemDescription'})
    final=final.fillna('')
    return final

def Filtration(df,KeepMostSimilarDescription=False):
    names=df.copy()
    names['company name']=names['name']
    names=names.sort_values(by='company name').reset_index(drop=True)
    result=searchListCompaniesInKg(names).sort_values(by='company name').reset_index(drop=True)
    if KeepMostSimilarDescription:
        ...
        if(len(result)==1):
            ...
            names1=addNameOnTop(result,names)
            
            #names1=names.groupby(["company name"]).apply(lambda x :addNameOnTop(searchListCompaniesInKg(x),x))
            
            try:   
                result=names1.groupby(["company name"]).apply(lambda x :KeepMostSimilarDes(x))
                result=result.reset_index(drop=True)
